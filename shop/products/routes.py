...
def deletebrand(id):
    ...
    flash(f'The brand {brand.name} cant be deleted from database', 'warning')
    return redirect(url_for('admin'))
...
def deletecategory(id):
    ...
    if request.method == "POST":
        ...
        return redirect(url_for('admin'))
    flash(f'The category {category.name} cant be deleted from database', 'warning')
    return redirect(url_for('admin'))
...
def addproduct():
    ...
    if request.method == "POST":
        ...
        addpro = Addproduct(name=name, price=price, discount=discount, stock=stock, colors=colors, desc=desc, brand_id=brand, category_id=category, image=image)
...
def deleteproduct(id):
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        flash(f'The product has been deleted', 'success')
        return redirect(url_for('admin'))
    flash('Cant be deleted', 'danger')
    return redirect(url_for('admin'))
